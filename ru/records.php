...
$back = $app->add(['Label','Назад ','green left ribbon','iconRight'=>'arrow left'])->link(['main']);

$app->add(['ui'=>'hidden divider']);

$back = $app->add(['Label','Выйти ','red right ribbon','iconRight'=>'sign out'])->link(['../logout']);
  ...
  if($records->tryLoadAny()->loaded()) {
    ...
    foreach($records as $rowss) {
      if ($rowss['patient_id'] == $_SESSION['id']) {
        ...
        $app->add(['Header',"Время: ".$rowss['time']." ,кабинет: ".$rowss['room']." ,врач: ".$doctor['name'],'green']);
      }
    }
  } else {
    $app->add(['Header',"У вас пока нет записей.",'green']);
  }
} else {
