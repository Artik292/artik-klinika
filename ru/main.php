...
require '../app.php';
$app = new App();
$to_record = $app->add(['Label','Просмотреть мои записи','blue left ribbon'])->link(['records']);

$app->add(['ui'=>'hidden divider']);

$logout = $app->add(['Label','Выйти ','red right ribbon','iconRight'=>'sign out'])->link(['../logout']);
...
$c3 = $col->addColumn();
$c4 = $col->addColumn();
$mes = $c4->add(['Message','Инструкция','massive info']);
$mes->text->addParagraph('Выберите направление, затем врача и время. После нажмите "подтвердить".');
...
if($t) {
  ...
  if ($doctor['available']) {
    ...
    $menu->addHeader('Время ('.$doctor['name'].')');

    $vir = $app->add('VirtualPage');
    $vir->set(function($vir) use($parentss,$db,$app,$doctor_id,$t) {
      $mes = $vir->add(['Message','Подтвердите выбор','massive info']);
      ...
      $mes->text->addParagraph('Доктор: '.$doctor_name);
      $mes->text->addParagraph('Направление: '.$specifik);
      $mes->text->addParagraph('Дата: '.$time);
      $form = $vir->add('Form');
      $form->buttonSave->set('Подтвердить');
      $form->onSubmit(function($form) use($app,$t,$db,$time,$room,$doctor_id) {
        ...
        $record->save();
        return [$form->success('Запись прошла успешна.') , new \atk4\ui\jsExpression('document.location="main.php"')];
...
$app->add(['Label','Это приложение было создано Артуром Коневником ak19192 для LUDF ','green right ribbon'])->link('https://github.com/Artik292');
